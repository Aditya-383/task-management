...
const TaskUpdateForm = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.patch(`http://localhost:5000/api/task/updateByAdmin/${taskId}`, taskData, {
        headers: {
      ...
      <section className="w-full max-w-3xl bg-white    p-6 md:p-10 rounded-2xl shadow-md">
        ...
        <form onSubmit={handleFormSubmit} >
          <div>
            ...
            <input  type="text"  id="taskName" value={taskName} onChange={(e) => setTaskName(e.target.value)} className="w-full px-4 py-2 border border-blue-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input  type="date" id="startDate" value={startDate} onChange={(e) => setStartDate(e.target.value)}
                className="w-full px-4 py-2 border border-[#1d3557] rounded-lg focus:outline-none"
              />
            </div>
